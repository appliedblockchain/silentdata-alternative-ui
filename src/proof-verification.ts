...
const uuidBytes = (uuid: string): ArrayBuffer => hexToArrayBuffer(uuid.replace(/-/g, ''))

function minimumBalanceBinaryAttestationData(proof: MinimumBalanceProof): ArrayBuffer {
...
function consistentIncomeBinaryAttestationData(proof: ConsistentIncomeProof): ArrayBuffer {
  const data = new ArrayBuffer(346 + proof.typeSpecificData.certificateChain.length)
  const uints = new Uint8Array(data)
  const dataView = new DataView(data)
...
function accountOwnershipBinaryAttestationData(proof: AccountOwnershipProof): ArrayBuffer {
